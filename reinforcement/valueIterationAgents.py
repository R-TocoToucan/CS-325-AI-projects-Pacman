...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        # Q * (s, a) = sum T(s,a,s')[R(s, a, s') + gV*(s')]

        value = 0.0
        for nextState, action in self.mdp.getTransitionStatesAndProbs(state, action):
            reward = self.mdp.getReward(state, action, nextState)
            value += action * (reward + self.discount * self.values[nextState])
